...
import { Node, Comment, SourceLocation } from './estree';
...
/**
 * The interface that is emitted for the`onToken`
 * handler.
 */
export interface TokenEmitType {
	type: string,
	start: number,
	value: any,
	end: number,
 ...
 */
export type OnToken = (token: TokenEmitType) => void;
...
export interface ParserState {
  ...
  token: Token;
  onComment: any;
  onToken: (token: TokenEmitType) => void;
  tokenValue: any;
  tokenRaw: string;
  tokenRegExp: void | {
    pattern: string;
...
export function pushToken(_: Context, array: TokenEmitType[]) {
  return (token: TokenEmitType)  => array.push(token);
}
